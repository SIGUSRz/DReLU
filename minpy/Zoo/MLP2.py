...
class mlp:
    def __init__(self,**kwargs):
        ...
        self.param, self.drelu_pos = gaussian_random(structure=self.structure,mode=self.act_mode,weight=kwargs['weight_init'],bias=kwargs['bias_init'],lower=kwargs['lower_bound'],upper=kwargs['upper_bound'])
    ...
    def _forward(self, X, *args):
        ...
        if flags.RECORD_FLAG:
            parameter_records = {
                ...
                'w2': w2.asnumpy(),
                'b2': b2.asnumpy(),
                'lower1': lower1.asnumpy() if lower1 is not None else None,
                'upper1': upper1.asnumpy() if upper1 is not None else None,
                'lower2': lower2.asnumpy() if lower2 is not None else None,
                'upper2': upper2.asnumpy() if upper2 is not None else None
            }
